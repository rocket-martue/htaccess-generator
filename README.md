...
├── js/
│   ├── main.js             ← エントリーポイント（イベント・テーマ・コピー・ダウンロード）
...
スタティックサイトのためビルド不要。任意の HTTP サーバーでルートを配信するだけで動作する。

```bash
# 例: VS Code Live Server や Python の簡易サーバー
python -m http.server 8000
```

SCSS の編集には VS Code 拡張機能 **Live Sass Compiler** を使用。`scss/style.scss` を保存すると `css/style.css` に自動コンパイルされる。
