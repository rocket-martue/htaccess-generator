...
| 管理画面 CSP（wp-admin / wp-login.php） | フロント CSP と同じディレクティブ構成をベースに script-src / style-src へ `'unsafe-inline'` / `'unsafe-eval'` を追加して動的生成 | ハードコード定数を廃止し、ユーザー設定を反映しつつ wp-admin の動作に必要な unsafe-* を保証するため |
